...
use anchor_spl::{
    associated_token::AssociatedToken, token_interface::{transfer_checked, Mint, TokenAccount, TokenInterface,CloseAccount,TransferChecked,close_account}
};
...
pub fn withdraw_and_close_vault(ctx: Context<TakeOffer>) -> Result<()> {
    ...
    let cpi_context = 
        CpiContext::new_with_signer(ctx.accounts.token_program.to_account_info(),accounts,&signer_seeds);
    
    _ = transfer_checked(cpi_context, ctx.accounts.vault.amount, ctx.accounts.token_mint_a.decimals);
